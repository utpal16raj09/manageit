...
export const DesktopSidebar: React.FC = () => {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-white text-[#012169] fixed top-0 bottom-0 left-0 z-40 font-sans shadow-sm">
      {/* Clean Brand Header */}
      <div className="p-6 flex items-center justify-between">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab(activeRole === 'tenant' ? 'tenant-dashboard' : activeRole === 'manager' ? 'manager-dashboard' : 'dashboard')}>
        ...
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between px-3.5 py-3 rounded-2xl font-extrabold text-sm transition-all duration-150 gap-2 cursor-pointer ${
                isActive
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <Icon className={`w-5 h-5 flex-shrink-0 stroke-[2] ${isActive ? 'text-white' : 'text-[#009cde]'}`} />
                <span className="truncate">{item.label}</span>
              </div>

              {item.badge !== undefined && item.badge > 0 && (
                <span className={`px-2 py-0.5 rounded-md text-xs font-black flex-shrink-0 ${
        ...
        {activeRole !== 'tenant' && (
          <>
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl font-extrabold text-sm text-slate-600 hover:bg-[#f0f7ff] hover:text-[#003087] transition-all gap-2 cursor-pointer"
            >
              <div className="flex items-center gap-3 min-w-0">
                <ScribbleQR className="w-5 h-5 text-[#009cde] flex-shrink-0" />
                <span className="truncate">Tenant QR Pass</span>
              </div>
      ...
      <div className="p-3.5 bg-[#f8fafc] space-y-2">
        <div
          onClick={() => setActiveTab('settings')}
          className="flex items-center justify-between p-2.5 rounded-2xl cursor-pointer hover:bg-white hover:shadow-xs transition-all duration-200 group"
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-[#003087] text-white flex items-center justify-center font-extrabold text-xs flex-shrink-0 shadow-xs">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11px] font-semibold text-slate-400 truncate capitalize">
                {activeRole ? (activeRole === 'owner' ? 'Property Owner' : activeRole === 'manager' ? 'Operations Manager' : 'Resident Tenant') : (currentUser.role === 'owner' ? 'Property Owner' : currentUser.role)}
              </div>
            </div>
          </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 p-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-extrabold text-xs transition-all duration-200 shadow-2xs cursor-pointer"
        >
